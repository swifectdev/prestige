...
Public Class tpc
    ...
    Private Sub initializedg()
        ...
        With dgview
            ...
            .Columns(2).Name = "date_trans" : .Columns(2).HeaderText = "Tgl Inv"
            .Columns(3).Name = "consignee" : .Columns(3).HeaderText = "Consignee/Owner"
            ...
            .Columns(7).Name = "hrgjual" : .Columns(7).HeaderText = "Hrg Jual"
            ...
            .Columns(12).Name = "code_mconsignee" : .Columns(12).Visible = False
            .Columns(13).Name = "note" : .Columns(13).HeaderText = "Note/Catatan"
        End With
        adjcolumn()
    End Sub
    ...
    Private Sub adjcolumn()
        ...
        With dgview
            ...
            .Columns("hrgjual").DefaultCellStyle.BackColor = Color.MistyRose
    ...
    Sub CellValidation(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
        ...
        ElseIf colname <> "paytotal" And colname <> "note" Then
            e.KeyChar = ""
        End If
    End Sub
    ...
    Private Sub btnsave_Click(sender As System.Object, e As System.EventArgs) Handles btnsave.Click
        With cl
            ...
            If btnsave.Text = "Save" Then
                ...
                If .readData("SELECT COUNT(id) FROM tpc WHERE no = '" & txtno.Text & "' AND tstatus = 1") > 0 Then
                ...
                '        .execCmdTrans(
                '             "EXEC stpcskminj 'I', '" & .getDataTrans("SELECT TOP 1 id FROM tpayh ORDER BY id DESC") & "', '" & idusr & "'")
                '    ElseIf cl.cnum(txtexratediff.Text) > 0 Then
                '        .execCmdTrans(
                '             "EXEC stpcskplsj 'I', '" & .getDataTrans("SELECT TOP 1 id FROM tpayh ORDER BY id DESC") & "', '" & idusr & "'")
                '    End If
                'End If
            ...
            ElseIf btnsave.Text = "Update" Then
                '----------USER AUTHORIZATION CHECK--------------'
                If .readData("SELECT canupdate from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'tpc'") = "N" Then
                    .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                    Exit Sub
                End If
                '---------------------END------------------------'
    ...
    Private Sub btnlist_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnlist.Click
        ...
        sqlStr =
            "SELECT T0.* " &
            " FROM tpc T0 WHERE T0.tstatus = 1 " & sqlwhere
        ...
        With a.dgview : .DataSource = cl.table(sqlStr & " order by tdate DESC")
            ...
            .Columns("code_mconsignee").Visible = True : .Columns("code_mconsignee").HeaderText = "Customer/Buyer"
        ...
        a.loadStatusTempForm(Me, Me.btnlist, "[tpc]tpc")
        a.loadSQLSearch(sqlStr, 1)
        a.Text = "Pencarian : Payment To Consignee/Owner"
    ...
    Private Sub btndelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btndelete.Click
        With cl
            '----------USER AUTHORIZATION CHECK--------------'
            If .readData("SELECT candelete from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'tpc'") = "N" Then
                .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                Exit Sub
            End If
            '---------------------END------------------------'
    ...
    Private Sub btnbrowsecust_Click(sender As Object, e As EventArgs) Handles btnbrowsecust.Click
        ...
        With a.dgview : .DataSource = cl.table(sqlStr)
            ...
            .Columns("norek").Visible = True : .Columns("norek").HeaderText = "No Rekening"
        End With

        a.loadStatusTempForm(Me, Me.txtname_mcust, "[mconsignee]tpc")
        a.loadSQLSearch(sqlStr, 1)
    ...
    Public Sub getOutstandingInvoice()
        ...
        Dim dttable As DataTable = cl.table(
        "SELECT T0.no, T0.tdate, 'INVOICE' AS 'doctype_view', T2.name 'name_mconsignee', T2.hp1, T0.code_mitem, T1.name 'name_mitem', T0.hrgjual, " &
        "((T0.qty*T0.hrgbeli)+T0.profitbuyer)-ISNULL(dbo.fcgetpaycons(T0.no),0) 'total', T0.id 'idtrans', 'CNS' 'doctype', T0.code_mconsignee FROM tinvoice T0 INNER JOIN mitem T1 On T0.code_mitem = T1.code " &
        "INNER JOIN mconsignee T2 ON T0.code_mconsignee = T2.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T2.tstatus = 1 AND T0.code_mconsignee = '" & lblid_mconsignee.Text & "'" &
        " AND ((T0.qty*T0.hrgbeli)+T0.profitbuyer)-ISNULL(dbo.fcgetpaycons(T0.no),0) <> 0 AND (T0.qty*T0.hrgjual) - dbo.fcgetpayinv(T0.no) >= 0")

        If dttable.Rows.Count = 0 Then
            cl.msgError("Tidak ada Invoice Consignee outstanding !", "Information")
            lblid_mconsignee.Text = 0
            txtname_mcust.Text = ""
            Exit Sub
        Else
            dgview.Rows.Clear()
            ...
            For r As Integer = 0 To dttable.Rows.Count - 1
                ...
                With dttable
                    ...
                    dgview.Rows(rRow).Cells("consignee").Value = cl.cchr(.Rows(r).Item("name_mconsignee"))
                    ...
                    dgview.Rows(rRow).Cells("hrgjual").Value = cl.ccur(.Rows(r).Item("hrgjual"))
                    dgview.Rows(rRow).Cells("paytotal").Value = 0
                    dgview.Rows(rRow).Cells("doctype").Value = cl.cchr(.Rows(r).Item("doctype"))
                    dgview.Rows(rRow).Cells("code_mconsignee").Value = cl.cchr(.Rows(r).Item("code_mconsignee"))
    ...
    Public Sub getOutstandingInvoiceDate()
        ...
        Dim dttable As DataTable = cl.table(
        "SELECT T0.no, T0.tdate, 'INVOICE' AS 'doctype_view', T2.name 'name_mconsignee', T2.hp1, T0.code_mitem, T1.name 'name_mitem', " &
        "((T0.qty*T0.hrgbeli)+T0.profitbuyer)-ISNULL(dbo.fcgetpaycons(T0.no),0) 'total', T0.id 'idtrans', 'CNS' 'doctype', T0.code_mconsignee FROM tinvoice T0 INNER JOIN mitem T1 On T0.code_mitem = T1.code " &
        "INNER JOIN mconsignee T2 ON T0.code_mconsignee = T2.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T2.tstatus = 1 " &
        " AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND ((T0.qty*T0.hrgbeli)+T0.profitbuyer)-ISNULL(dbo.fcgetpaycons(T0.no),0) <> 0 AND (T0.qty*T0.hrgjual) - dbo.fcgetpayinv(T0.no) >= 0")
    ...
    Private Sub btnbrowsecoa_Click(sender As Object, e As EventArgs) Handles btnbrowsecoa.Click
        ...
        a.loadStatusTempForm(Me, Me.txtname_mcust, "[maccount]tpc")
        a.loadSQLSearch(sqlStr, 1)
