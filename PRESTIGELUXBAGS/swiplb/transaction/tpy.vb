...
Public Class tpy
    ...
    Private Sub gencode()
        txtno.Text = cl.readData("SELECT dbo.fgetcode('tpy','" & Format(CDate(dttdt.Text), "yy") & "')")
    End Sub
    ...
    Private Sub initializedg()
        ...
        With dgview
            .Columns(1).Name = "no_trans" : .Columns(1).HeaderText = "No. Inv"
            .Columns(2).Name = "doctype_view" : .Columns(2).HeaderText = "Jenis Trans"
            .Columns(3).Name = "date_trans" : .Columns(3).HeaderText = "Tgl Inv"
            .Columns(4).Name = "customer" : .Columns(4).HeaderText = "Customer/Buyer"
            .Columns(5).Name = "hp" : .Columns(5).HeaderText = "HP"
            .Columns(6).Name = "code_mitem" : .Columns(6).Visible = False
            .Columns(7).Name = "name_mitem" : .Columns(7).Visible = False
            .Columns(8).Name = "total" : .Columns(8).HeaderText = "Total/Sisa Total"
            .Columns(9).Name = "ongkir" : .Columns(9).HeaderText = "O. Kirim"
            .Columns(10).Name = "paytotal" : .Columns(10).HeaderText = "Total Bayar"

            .Columns(11).Name = "id_trans" : .Columns(11).Visible = False
            .Columns(12).Name = "doctype" : .Columns(12).Visible = False
            .Columns(13).Name = "code_mcust" : .Columns(13).Visible = False
        End With
        adjcolumn()
    End Sub
    ...
    Private Sub adjcolumn()
        ...
        With dgview
            ...
            .Columns("ongkir").DefaultCellStyle.BackColor = Color.PaleGreen
    ...
    Sub CellValidation(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
        ...
        If colname = "paytotal" Or colname = "ongkir" Then
        ...
        ElseIf colname <> "paytotal" And colname <> "ongkir" Then
            e.KeyChar = ""
        End If
    End Sub
    ...
    Private Sub btnsave_Click(sender As System.Object, e As System.EventArgs) Handles btnsave.Click
        With cl
            ...
            If btnsave.Text = "Save" Then
                ...
                If .readData("SELECT canadd from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'tpy'") = "N" Then
                ...
                If .readData("SELECT COUNT(id) FROM tpy WHERE no = '" & txtno.Text & "' AND tstatus = 1") > 0 Then
                ...
                For i As Integer = 0 To dgview.Rows.Count - 1
                    If .cnum(dgview.Rows(i).Cells("paytotal").Value) <> 0 Then
                        .execCmdTrans(
                            ...
                            " , '" & .cchr(dgview.Rows(i).Cells("code_mcust").Value) & "'" &
                        ...
                        '-- SET AND UPDATE DOCSTAT INVOICE
                        '   .execCmdTrans("UPDATE tinvoice SET docstat = 'P' WHERE no = '" & .cchr(dgview.Rows(i).Cells("no_trans").Value) & "' and tstatus = 1")
                    End If
                Next
                ...
                'If cl.getDataTrans("SELECT strvl FROM csetting WHERE name = 'autopost'") = "Y" And
                        ...
                        '        .execCmdTrans(
                        '             "EXEC stpyskminj 'I', '" & .getDataTrans("SELECT TOP 1 id FROM tpayh ORDER BY id DESC") & "', '" & idusr & "'")
                        '    ElseIf cl.cnum(txtexratediff.Text) > 0 Then
                        '        .execCmdTrans(
                        '             "EXEC stpyskplsj 'I', '" & .getDataTrans("SELECT TOP 1 id FROM tpayh ORDER BY id DESC") & "', '" & idusr & "'")
                        '    End If
                        'End If
            ...
            ElseIf btnsave.Text = "Update" Then
                '----------USER AUTHORIZATION CHECK--------------'
                If .readData("SELECT canupdate from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'tpy'") = "N" Then
                    .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                    Exit Sub
                End If
                '---------------------END------------------------'
                ...
                '    If cl.cnum(txtexratediff.Text) < 0 Then
                '        .execCmdTrans(
                '             "EXEC stpyskminj 'I', '" & idmaster & "', '" & idusr & "'")
                '    ElseIf cl.cnum(txtexratediff.Text) > 0 Then
                '        .execCmdTrans(
                '             "EXEC stpyskplsj 'I', '" & idmaster & "', '" & idusr & "'")
                '    End If
    ...
    Private Sub btnlist_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnlist.Click
        ...
        'sqlStr =
        '    "SELECT T0.* " &
        '    " FROM tpy T0 WHERE T0.tstatus = 1 " & sqlwhere

        sqlStr =
            "SELECT T0.id, T0.no, T0.tdate, T0.paytype, T1.code_mcust, T2.name 'name_mcust', T2.hp1, T0.code_maccount, T1.paytotal 'total', T0.note " &
            " FROM tpy T0 INNER JOIN tpyd T1 ON T0.id = T1.id_tpyh INNER JOIN mcust T2 ON T1.code_mcust = T2.code " &
            " WHERE T0.tstatus = 1 And T1.tstatus = 1 " & sqlwhere
        ...
        With a.dgview : .DataSource = cl.table(sqlStr & " order by tdate DESC")
            ...
            '.Columns("code_mcust").Visible = True : .Columns("code_mcust").HeaderText = "Kode Cust/Buyer"
            ...
            .Columns("note").Visible = True : .Columns("note").HeaderText = "Catatan"
        End With

        a.loadStatusTempForm(Me, Me.btnlist, "[tpy]tpy")
        a.loadSQLSearch(sqlStr, 1)
        a.Text = "Pencarian : Payment from Customer"
    ...
    Private Sub btndelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btndelete.Click
        With cl
            '----------USER AUTHORIZATION CHECK--------------'
            If .readData("SELECT candelete from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'tpy'") = "N" Then
                .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                Exit Sub
            End If
            '---------------------END------------------------'
            Dim tny As Integer
            tny = .msgYesNo("Apakah anda mau menghapus Pembayaran dari Customer : " & txtno.Text & " ?", "Konfirmasi")
            If tny = vbYes Then
                .openTrans()
                ...
                For i As Integer = 0 To dgview.Rows.Count - 1
                    If .cnum(dgview.Rows(i).Cells("paytotal").Value) <> 0 Then
                        '-- SET AND UPDATE DOCSTAT INVOICE
                        .execCmdTrans("UPDATE tinvoice SET docstat = 'O' WHERE no = '" & .cchr(dgview.Rows(i).Cells("no_trans").Value) & "' and tstatus = 1")
                    End If
                Next
                ...
                .closeTrans(btnsave)
                If .sCloseTrans = 1 Then
                    .msgInform("Sukses menghapus Pembayaran dari Customer : " & txtno.Text & " !", "Informasi")
                    changestatform("new") : End If
            End If
        End With
    End Sub
    ...
    Private Sub btnbrowsecust_Click(sender As Object, e As EventArgs) Handles btnbrowsecust.Click
        ...
        a.loadStatusTempForm(Me, Me.txtname_mcust, "[mcust]tpy")
        a.loadSQLSearch(sqlStr, 1)
    ...
    Public Sub getOutstandingInvoice()
        ...
        'Dim dttable As DataTable = cl.table(
        '"SELECT T0.no, T0.tdate, 'INVOICE' AS 'doctype_view', T0.name_mcust, T0.hp1, T0.code_mitem, T1.name 'name_mitem', ((T0.qty*T0.hrgjual))-ISNULL(dbo.fcgetpayinv(T0.no),0) 'total', T0.id 'idtrans', 'INV' 'doctype', T0.code_mcust FROM tinvoice T0 " &
        '"INNER JOIN mitem T1 On T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.code_mcust = '" & lblid_mcust.Text & "' AND ((T0.qty*T0.hrgjual))-ISNULL(dbo.fcgetpayinv(T0.no),0) <> 0 AND ISNULL(T0.docstat,'O') = 'O' UNION ALL " &
        '"SELECT T0.no, T0.tdate, 'INVOICE JASA' AS 'doctype_view', T1.name 'name_mcust', T1.hp1, T0.note, T0.note, T0.total, T0.id, 'INJ' 'doctype', T0.code_mcust FROM tinvj T0 " &
        '"INNER JOIN mcust T1 On T0.code_mcust = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.code_mcust = '" & lblid_mcust.Text & "' AND T0.total - dbo.fcgetpayinj(T0.no) <> 0 UNION ALL " &
        '"SELECT T0.no, T0.tdate, 'PRE-ORDER' AS 'doctype_view', T0.name_mcust, T0.hp1, T0.code_mitem, T0.code_mitem, (T0.hrgjual*T0.qty)-dbo.fcgetpaypo(T0.no), T0.id, 'PO' 'doctype', T0.code_mcust FROM tpo T0 " &
        '"INNER JOIN mitem T1 ON T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.code_mcust = '" & lblid_mcust.Text & "' AND (T0.qty*T0.hrgjual) - dbo.fcgetpaypo(T0.no) <> 0")

        Dim dttable As DataTable = cl.table(
        "SELECT T0.no, T0.tdate, 'INVOICE' AS 'doctype_view', T0.name_mcust, T0.hp1, ISNULL(T0.grdtotal,0)-ISNULL(dbo.fcgetpayinv(T0.no),0) 'total'," &
        " T0.id 'idtrans', 'INV' 'doctype', T0.code_mcust FROM tinvoice T0 " &
        "INNER JOIN mitem T1 On T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.code_mcust = '" & lblid_mcust.Text & "' AND ISNULL(T0.grdtotal,0)-ISNULL(dbo.fcgetpayinv(T0.no),0) <> 0 AND ISNULL(T0.docstat,'O') = 'O' " &
        " GROUP BY T0.no, T0.tdate, T0.name_mcust, T0.hp1, T0.grdtotal, T0.id, T0.code_mcust UNION ALL " &
        "SELECT T0.no, T0.tdate, 'INVOICE JASA' AS 'doctype_view', T1.name 'name_mcust', T1.hp1, T0.note, T0.note, T0.total, T0.id, 'INJ' 'doctype', T0.code_mcust FROM tinvj T0 " &
        "INNER JOIN mcust T1 On T0.code_mcust = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.code_mcust = '" & lblid_mcust.Text & "' AND T0.total - dbo.fcgetpayinj(T0.no) <> 0 UNION ALL " &
        "SELECT T0.no, T0.tdate, 'PRE-ORDER' AS 'doctype_view', T0.name_mcust, T0.hp1, (T0.hrgjual*T0.qty)-dbo.fcgetpaypo(T0.no), T0.id, 'PO' 'doctype', T0.code_mcust FROM tpo T0 " &
        "INNER JOIN mitem T1 ON T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.code_mcust = '" & lblid_mcust.Text & "' AND (T0.qty*T0.hrgjual) - dbo.fcgetpaypo(T0.no) <> 0")
        ...
        Else
            ...
            For r As Integer = 0 To dttable.Rows.Count - 1
                ...
                With dttable
                    ...
                    dgview.Rows(rRow).Cells("doctype_view").Value = cl.cchr(.Rows(r).Item("doctype_view"))
                    dgview.Rows(rRow).Cells("date_trans").Value = cl.cchr(.Rows(r).Item("tdate"))
                    dgview.Rows(rRow).Cells("customer").Value = cl.cchr(.Rows(r).Item("name_mcust"))
                    ...
                    dgview.Rows(rRow).Cells("code_mcust").Value = cl.cchr(.Rows(r).Item("code_mcust"))
    ...
    Public Sub getOutstandingInvoiceDate()
        'Dim dttable As DataTable = cl.table(
        '"SELECT T0.id, T0.no, T0.tdt, T0.refno, T2.name AS 'name_mcust', T0.grdtotal 'total', " &
        '" SUM(T0.grdtotal)-ISNULL(SUM(T3.paytotal),0)-ISNULL((SELECT SUM(payment) FROM tpayd TX WHERE TX.idtrans = T0.id AND TX.tstatus = 1),0)-  " &
        '" ISNULL((SELECT SUM(grdtotal*-1) FROM treth TY WHERE TY.id_tsalh = T0.id AND TY.tstatus = 1),0) 'sisatotal' " &
        ...
        'Dim dttable As DataTable = cl.table(
        '"SELECT T0.no, T0.tdate, 'INVOICE' AS 'doctype_view', T0.name_mcust, T0.hp1, T0.code_mitem, T1.name 'name_mitem', ((T0.qty*T0.hrgjual))-ISNULL(dbo.fcgetpayinv(T0.no),0) 'total', T0.id 'idtrans', 'INV' 'doctype', T0.code_mcust FROM tinvoice T0 " &
        '"INNER JOIN mitem T1 On T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND ((T0.qty*T0.hrgjual))-ISNULL(dbo.fcgetpayinv(T0.no),0) <> 0 AND ISNULL(T0.docstat,'O') = 'O' UNION ALL " &
        '"SELECT T0.no, T0.tdate, 'INVOICE JASA' AS 'doctype_view', T1.name 'name_mcust', T1.hp1, T0.note, T0.note, T0.total, T0.id, 'INJ' 'doctype', T0.code_mcust FROM tinvj T0 " &
        '"INNER JOIN mcust T1 On T0.code_mcust = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND T0.total - dbo.fcgetpayinj(T0.no) <> 0 UNION ALL " &
        '"SELECT T0.no, T0.tdate, 'PRE-ORDER' AS 'doctype_view', T0.name_mcust, T0.hp1, T0.code_mitem, T0.code_mitem, (T0.hrgjual*T0.qty)-dbo.fcgetpaypo(T0.no), T0.id, 'PO' 'doctype', T0.code_mcust FROM tpo T0 " &
        '"INNER JOIN mitem T1 ON T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND (T0.qty*T0.hrgjual) - dbo.fcgetpaypo(T0.no) <> 0")

        Dim dttable As DataTable = cl.table(
        "SELECT T0.no, T0.tdate, 'INVOICE' AS 'doctype_view', T0.name_mcust, T0.hp1, ISNULL(T0.grdtotal,0)-ISNULL(dbo.fcgetpayinv(T0.no),0) 'total'," &
        " 'INV' 'doctype', T0.code_mcust FROM tinvoice T0 " &
        "INNER JOIN mitem T1 On T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 " &
        " AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND  ISNULL(T0.grdtotal,0)-ISNULL(dbo.fcgetpayinv(T0.no),0) <> 0 And ISNULL(T0.docstat,'O') = 'O' " &
        " GROUP BY T0.no, T0.tdate, T0.name_mcust, T0.hp1, T0.grdtotal, T0.code_mcust UNION ALL " &
        "SELECT T0.no, T0.tdate, 'INVOICE JASA' AS 'doctype_view', T1.name 'name_mcust', T1.hp1, T0.total, 'INJ' 'doctype', T0.code_mcust FROM tinvj T0 " &
        "INNER JOIN mcust T1 On T0.code_mcust = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND T0.total - dbo.fcgetpayinj(T0.no) <> 0 UNION ALL " &
        "SELECT T0.no, T0.tdate, 'PRE-ORDER' AS 'doctype_view', T0.name_mcust, T0.hp1,  (T0.hrgjual*T0.qty)-dbo.fcgetpaypo(T0.no), 'PO' 'doctype', T0.code_mcust FROM tpo T0 " &
        "INNER JOIN mitem T1 ON T0.code_mitem = T1.code WHERE T0.tstatus = 1 And T1.tstatus = 1 AND T0.tdate BETWEEN '" & Format(dtfrom.Value, "yyyyMMdd") & "' AND '" & Format(dtto.Value, "yyyyMMdd") & "' AND (T0.qty*T0.hrgjual) - dbo.fcgetpaypo(T0.no) <> 0")
    ...
    Private Sub btnbrowsecoa_Click(sender As Object, e As EventArgs) Handles btnbrowsecoa.Click
        ...
        a.loadStatusTempForm(Me, Me.txtname_mcust, "[maccount]tpy")
        a.loadSQLSearch(sqlStr, 1)
    ...
    Private Sub dgview_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgview.CellDoubleClick
        ...
        sqlStr =
            "SELECT * FROM tinvoice where no = '" & dgview.Item("no_trans", dgview.CurrentRow.Index).Value & "' and tstatus = 1"
        ...
        With a.dgview : .DataSource = cl.table(sqlStr & " order by tdate DESC")
            ...
            .Columns("code_mcurrj").Visible = True : .Columns("code_mcurrj").HeaderText = "Mata Uang"
            '.Columns("code_mcust").Visible = True : .Columns("code_mcust").HeaderText = "Kode Cust/Buyer"
            .Columns("hrgjual").Visible = True : .Columns("hrgjual").HeaderText = "Hrg Jual" : .Columns("hrgjual").DefaultCellStyle.Format = "n2"

        End With

        a.loadStatusTempForm(Me, Me.btnlist, "[tpy]tpy")
        a.loadSQLSearch(sqlStr, 1)
        a.Text = "Rincian Item Invoice : '" & dgview.Item("no_trans", dgview.CurrentRow.Index).Value & "'"

        a.BtnSave.Enabled = False
        a.btnExcel.Enabled = False
    ...
    '    a.loadStatusTempForm(Me, Me.txtname_mcust, "[mbp]tpy")
    '    a.loadSQLSearch(sqlStr, 1)
    ...
    '    a.loadStatusTempForm(Me, Me.dgview, "[mcoa]tpy")
    '    a.loadSQLSearch(sqlstr, 1)
