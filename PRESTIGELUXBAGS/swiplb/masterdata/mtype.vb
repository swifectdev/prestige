...
Public Class mtype
    ...
    Private Sub gencode()
        '   txtcode.Text = cl.readData("SELECT dbo.fgetcode('maccount','" & Now() & "')")
    End Sub
    ...
    Private Sub btnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsave.Click
        With cl
            ...
            If btnsave.Text = "Save" Then
                ...
                If .readData("SELECT canadd from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'mtype'") = "N" Then
                    .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                    Exit Sub
                End If

                If cl.readData("SELECT COUNT(id) FROM mtype where code = '" & txtcode.Text & "' and tstatus = 1") Then
                    cl.msgError("Kode sudah ada, mohon untuk gunakan kode lain !", "Informasi")
                    Exit Sub
                End If
            ...
            ElseIf btnsave.Text = "Update" Then
                '----------USER AUTHORIZATION CHECK--------------'
                If .readData("SELECT canupdate from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'mtype'") = "N" Then
                    .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                    Exit Sub
                End If
                '---------------------END------------------------'
    ...
    Private Sub btnlist_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnlist.Click
        ...
        Dim sql As String = " SELECT TA.* FROM mtype TA WHERE TA.tstatus = 1"
        ...
        End With
        a.loadStatusTempForm(Me, Me.txtcode, "[mtype]mtype")
        a.loadSQLSearch(sql, 1)
        a.Text = "Pencarian Jenis Barang"
    ...
    Private Sub btndelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btndelete.Click
        With cl
            '----------USER AUTHORIZATION CHECK--------------'
            If .readData("SELECT candelete from cusrpriv WHERE id_cusr = '" & idusr & "' AND code_capp = 'maccount'") = "N" Then
                .msgError("User tidak dapat melakukan tindakan ini ! Mohon untuk check setting otorisasi !", "Informasi")
                Exit Sub
            End If
            '---------------------END------------------------'

            If cl.readData("SELECT COUNT(id) FROM mitem where code_mtype = '" & txtcode.Text & "' and tstatus = 1") Then
                cl.msgError("Kode sudah terkait dengan item master, tidak bisa delete !", "Informasi")
                Exit Sub
            End If

            Dim tny As Integer
            tny = .msgYesNo("Delete Jenis Barang : " & txtname.Text & " ?", "Confirmation")
            If tny = vbYes Then
                .openTrans()
                .execCmdTrans(
                        "EXEC smtype " &
                ...
                .closeTrans(btnsave)
                If .sCloseTrans = 1 Then
                    .msgInform("Success Delete Jenis Barang : " & txtname.Text & " !", "Information")
                    changestatform("new") : End If
            End If
        End With
    End Sub
